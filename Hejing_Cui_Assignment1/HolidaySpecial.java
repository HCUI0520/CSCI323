...
public class HolidaySpecial {
     ...
     * @param signUpTable An easy lookup tool, signUpTable[x][Y] = cook X signed
     * up or did not sign up for step Y. Example: signUpTable[1][3] = 1 if cook
     * 1 signed up for Step 3 signUpTable[1][3] = 0 if cook 1 didn't sign up for
     * Step 3
     * @return scheduleTable: a table similar to the signUpTable where
     * scheduleTable[X][Y] = 1 means cook X is assigned to step Y in an optimal
     * schedule
     */
    public int[][] makeShifts(
        ...
        int[] all_pass_steps = new int[numSteps + 1];
        int[] cook_pass = new int[numCooks + 1];
        //greedy algorithm for finding optimized solution
        while (true) {
            boolean all_pass = true;
            for(int i = 1; i <= numSteps; i++){
                if(all_pass_steps[i] == 0){
                    all_pass = false;
                    break;
                }
            }
            //if schedule is completed, finish greedy search
            if(all_pass) break;
            
            int cook_index = 0;
            int max_steps = 0;
            for (int i = 1; i <= numCooks; i++) {
                //if this cook is already done step, ignore this cook
                if (cook_pass[i] == 1) {
                    continue;
                }
                int[] sign_steps = signUpTable[i];
                int available_steps = 0;
                //find cook who can satisfy maximum steps
                for (int j = 1; j <= numSteps; j++) {
                    if (all_pass_steps[j] != 1 && sign_steps[j] == 1) {
                        available_steps++;
                    }
                }
                if (max_steps < available_steps) {
                    max_steps = available_steps;
                    cook_index = i;
                }
            }
            cook_pass[cook_index] = 1;
            //set steps
            int[] sign_steps = signUpTable[cook_index];
            for(int i = 1; i <= numSteps; i++){
                if(all_pass_steps[i] == 0 && sign_steps[i] == 1){
                    all_pass_steps[i] = 1;
                    scheduleTable[cook_index][i] = 1;
                }
            }
        }
    ...
    /**
     * Makes the convenient lookup table based on the steps each cook says they
     * can do
     *
     ...
     * lookupTable[x][y] = 1 if cook x can do step y lookupTable[x][y] = 0 if
     * cook x cannot do step y
     */
     ...
     * @return true or false whether the input sign-ups match the given number
     * of cooks and steps, and whether all the steps are guaranteed at least one
     * cook.
     */
     ...
     * schedule[x][y] means whether in the optimal schedule cook x is doing step
     * y
     */
    ...
    /**
     * You can make additional test cases using the same format. In fact the
     * helper functions I've provided will even check your test case is set up
     * correctly. Do not touch any of of the existing lines. Just make sure to
     * comment out or delete any of your own test cases when you submit. The
     * three recipe test cases existing in this main method should be the only
     * output when running this file.
     */
