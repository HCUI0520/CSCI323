...
/**
 * LeagueOfPatience Author: Hejing Cui and Carolyn Yao Does this compile or
 * finish running within 5 seconds? Y
 */
/**
...
public class LeagueOfPatience {
     ...
     * to T. It combines the given edge information about actual play time with
     * the time required to wait for each intermediary quest to become
     * available.
     *
     ...
     * @param durations durations[u][v] Table of how long game play between u
     * and v takes in minutes
     */
    public void myFastestPlay(
        ...
        List<Integer> visit_list = new ArrayList<Integer>();
        ...
        times[S] = 0;
        Date[] time_array = new Date[numVertices];
        time_array[S] = startTime;
        for (int count = 0; count < numVertices; count++) {
            ...
            for (int v = 0; v < numVertices; v++) {
                //start simulation from API
                Date next_date = getNextQuestTime(time_array[u], u, v);
                //get minutes between start time and next time
                int next_time = minutesBetween(time_array[u], next_date);
                //consider duration time and API time and get real time to take
                if (!processed[v] && durations[u][v] != 0 && times[u] != Integer.MAX_VALUE && times[u] + durations[u][v] + next_time < times[v]) {
                    times[v] = times[u] + durations[u][v] + next_time;
                    time_array[v] = getDurationDate(time_array[u], durations[u][v] + next_time);
                }
            }
        }
        printShortestTimes(times);
        //print visited route
        System.out.print("Visited Route: ");
        String route = "";
        for(int i = 0; i < visit_list.size() - 1; i++){
            route += visit_list.get(i) + ", ";
        }
        route += visit_list.get(visit_list.size() - 1);
    ...
    /**
     * This function is simulating the game's API that you can request the
     * closest quest start time from. For example, if you enter that you will
     * get to the quest at 13:45, it could give back something like 16:02 (Date
     * object) as the next quest time.
     *
    ...
    /**
     * get date by duration minute
     *
     * @param date
     * @param minutes
     * @return
     */
    public Date getDurationDate(Date date, int minutes) {
        Calendar calendar = Calendar.getInstance();
